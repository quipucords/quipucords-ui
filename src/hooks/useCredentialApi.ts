...
const useAddCredentialApi = (
  ...
  const callbackError = useCallback(
    (error: AxiosError<ApiCredentialErrorType>, name: string) => {
      ...
      if (onFieldErrors && error?.response?.data && apiHelpers.hasFieldValidationErrors(error.response.data)) {
...
export { 
  ...
  useAddCredentialApi,    // This is now the enhanced version
  useEditCredentialApi,   // This is now the enhanced version  
