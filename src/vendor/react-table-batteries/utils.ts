...
/**
 * Works around problems caused by event propagation when handling a clickable element that contains other clickable
 * elements. - Used internally by useTablePropHelpers for the active item feature,
 * but is generic and could be used outside tables. - When a click event happens within a row,
 * checks if there is a clickable element in between the target node and the row element.
