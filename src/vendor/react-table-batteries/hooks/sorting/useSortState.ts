...
export interface ActiveSort<TSortableColumnKey extends string> {
  /**
   * The identifier for the currently sorted column (`columnKey` values come from the keys of the `columnNames` object
   * passed to useTableState)
   */
...
export interface SortStateArgs<TItem, TSortableColumnKey extends string> extends FeatureStateCommonArgs {
  /**
   * The `columnKey` values (keys of the `columnNames` object passed to useTableState) corresponding to columns with
   * sorting enabled
   */
  ...
  /**
   * A callback function to return, for a given API data item,
   * a record of sortable primitives for that item's sortable columns - The record maps:
   ...
   * Added here even though it's not used in useSortState so args can be passed all at once. Actually used only in
   * useClientSortDerivedState.
   *
 ...
 * - Included in the `TableState` object returned by useTableState under the `sort` sub-object (combined with args
 * above). - Also included in the `TableBatteries` object returned by useTablePropHelpers and useClientTableBatteries.
 *
 * @see TableState
 * @see TableBatteries
 */
...
export const useSortState = <TItem, TSortableColumnKey extends string, TPersistenceKeyPrefix extends string = string>(
  ...
  const initialSort: ActiveSort<TSortableColumnKey> | null = args.sort?.initialSort ?? (sortableColumns[0]
