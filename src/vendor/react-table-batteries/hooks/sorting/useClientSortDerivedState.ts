...
export interface UseClientSortDerivedStateArgs<TItem, TSortableColumnKey extends string> {
  ...
  items: TItem[];
  /**
   * Feature-specific args: A subset of the `TableState` object's `sort` property with the state itself and relevant
   * state args
   */
...
/**
 * Given the "source of truth" state for the sort feature and additional arguments,
 * returns "derived state" values and convenience functions. - For local/client-computed tables only.
 * Performs the actual sorting logic, which is done on the server for server-computed tables.
