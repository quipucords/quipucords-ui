...
export const useExpansionPropHelpers = <TItem, TColumnKey extends string>(
  ...
  /**
   * Returns props for the Td to the left of the data cells which contains each row's expansion toggle button (only for
   * single-expand).
   */
  ...
  /**
   * Returns props for the Td which is a data cell in an expandable column and functions as an expand toggle (only for
   * compound-expand)
   */
  ...
  /**
   * Returns props for the Td which contains the expanded content below an expandable row (for both single-expand and
   * compound-expand). This Td should be rendered as the only cell in a Tr just below the Tr containing the
   * corresponding row.
   * The Tr for the row content and the Tr for the expanded content should be the only two children of a Tbody grouping
   * them (one per expandable row).
   */
