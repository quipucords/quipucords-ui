...
export interface UseSelectionDerivedStateArgs<TItem> {
  /**
   * The string key/name of a property on the API data item objects that can be used as a unique identifier (string or
   * number)
   */
  ...
  items?: TItem[];
  /**
   * Feature-specific args: A subset of the `TableState` object's `selection` property with the state itself and
   * relevant state args
   */
 ...
 * - "Derived state" here refers to values and convenience functions derived at render time based on the "source of
 * truth" state. - "source of truth" (persisted) state and "derived state" are kept separate to prevent out-of-sync
 * duplicated state.
 */
export interface SelectionDerivedState<TItem> {
  ...
  /**
   * Toggles selection on one item. Does not select an item that is not selectable (if an isItemSelectable callback is
   * being used).
   */
   ...
   * - Only works for client-paginated tables where we've passed in the `items` array containing all items (not just
   * the current page).
   */
  selectAll: () => void;
  /**
...
/**
 * Given the "source of truth" state for the selection feature and additional arguments,
 * returns "derived state" values and convenience functions.
 * - "source of truth" (persisted) state and "derived state" are kept separate to prevent out-of-sync duplicated state.
 *
 * NOTE: Unlike `useClient[Filter|Sort|Pagination]DerivedState`,
 * this is not named `useClientSelectionDerivedState` because it is always local/client-computed,
 * and it is still used when working with server-computed tables
  ...
  return {
    ...
    selectItems: (items, isSelecting = items.some(isItemSelected)) => {
      ...
      if (isSelecting && selectingItemIds.length > 0) {
        setSelectedItemIds(selected => [...selected.filter(id => !selectingItemIds.includes(id)), ...selectingItemIds]);
      } else if (!isSelecting) {
