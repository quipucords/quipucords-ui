...
export interface SelectionStateArgs<TItem> extends Omit<FeatureStateCommonArgs, 'persistTo'> {
   ...
   * Added here even though it's not used in useSelectionState so args can all be passed at once.
   * Actually used only in useSelectionDerivedState.
   */
 ...
 * - Included in the `TableState` object returned by useTableState under the `selection` sub-object (combined with args
