...
import { serializeFilterUrlParams, deserializeFilterUrlParams } from './helpers';
...
export interface FilterStateArgs<TItem, TFilterCategoryKey extends string> extends FeatureStateCommonArgs {
  /**
   * Definitions of the filters to be used (must include `getItemValue` functions for each category when performing
   * filtering locally)
   */
 ...
 * - Included in the `TableState` object returned by useTableState under the `filter` sub-object (combined with args
...
export interface FilterState<TFilterCategoryKey extends string> {
  /**
   * A mapping:
   * - from string keys uniquely identifying a filterCategory (inferred from the `key` properties of elements in the
   * `filterCategories` array) - to arrays of strings representing the current value(s) of that filter.
   * Single-value filters are stored as an array with one element.
   */
