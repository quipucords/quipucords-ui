...
export interface UseClientFilterDerivedStateArgs<TItem, TFilterCategoryKey extends string> {
  ...
  items: TItem[];
  /**
   * Feature-specific args: A subset of the `TableState` object's `filter` property with the state itself and relevant
   * state args
   */
...
/**
 * Given the "source of truth" state for the filter feature and additional arguments,
 * returns "derived state" values and convenience functions. - For local/client-computed tables only.
 * Performs the actual filtering logic, which is done on the server for server-computed tables.
