  ...
  const tableState = useTableState({
    ...
    filter: {
      ...
      filterCategories: [
        ...
        {
          key: API_QUERY_TYPES.SEARCH_REPORT_SCAN_ID,
          title: t('toolbar.label', { context: 'option_report_scan_id' }),
          type: FilterType.search,
          ...
          <Tbody>
            {currentPageItems?.map((report: ReportType, rowIndex) => (
              <TrWithBatteries key={report.id} item={report} rowIndex={rowIndex}>
                ...
                <Td columnKey="scan_id">{report.scan_id ?? t('table.label', { context: 'not-available' })}</Td>
                <Td columnKey="scan_name">{report.scan_name ?? t('table.label', { context: 'not-available' })}</Td>
