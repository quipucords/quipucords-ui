...
import { ActionGroup, Button, Form, FormGroup, FormHelperText, TextArea, TextInput, HelperText, HelperTextItem } from '@patternfly/react-core';
...
const useCredentialForm = ({
  ...
  const hasExistingValue = useCallback((field: string) => {
  ...
  // Get required fields based on auth type and edit mode
  ...
  const validateField = useCallback((field: string, value: string) => {
    ...
    // For edit mode, only validate if field is touched OR doesn't have existing value
    const shouldValidateAsRequired = requiredFields.includes(field) && 
      (!isEditMode || touchedFields.has(field) || !hasExistingValue(field));

    if (shouldValidateAsRequired && (!value || value.trim() === '')) {
      errors[field] = 'This field is required';
    }

    // Additional field-specific validation (only if field has a value)
  ...
  const validateForm = useCallback(() => {
    ...
    if (!isEditMode) {
      ...
      isFormValid = isFormValid && requiredFields.every(field => {
    ...
    return isFormValid;
  }, [formData, getRequiredFields, validateField, isEditMode, touchedFields, hasExistingValue]);
  ...
  // Validate form whenever formData, authType, or touchedFields changes
  useEffect(() => {
    validateForm();
  }, [formData, authType, touchedFields, validateForm]);
  ...
  const handleInputChange = useCallback(
    ...
    [formData, serverErrors, onClearErrors, validateField]
  );
...
// Enhanced ErrorFragment - simplified without the "leave blank" messaging
    ...
    <Form>
      <FormGroup label="Name" isRequired fieldId="name">
        ...
        <ErrorFragment 
          ...
          fieldTouched={touchedFields.has('name')}
        />
      </FormGroup>
      ...
      {authType === 'Token' && (
        <FormGroup label="Token" isRequired fieldId="auth_token">
          ...
          />
          <ErrorFragment 
            errorMessage={errors?.auth_token} 
            fieldTouched={touchedFields.has('auth_token')}
          />
        </FormGroup>
      )}
        ...
        <React.Fragment>
          <FormGroup label="Username" isRequired fieldId="username">
            ...
            <ErrorFragment 
              ...
              fieldTouched={touchedFields.has('username')}
            />
          </FormGroup>
          <FormGroup label="Password" isRequired fieldId="password">
            ...
            <ErrorFragment 
              ...
              fieldTouched={touchedFields.has('password')}
            />
          </FormGroup>
        </React.Fragment>
      )}
      ...
      {authType === 'SSH Key' && (
        <React.Fragment>
          ...
          <FormGroup label="SSH Key" isRequired fieldId="ssh_key">
            ...
            />
            <ErrorFragment 
              errorMessage={errors?.ssh_key} 
              fieldTouched={touchedFields.has('ssh_key')}
            />
          </FormGroup>
          <FormGroup label="SSH passphrase" fieldId="ssh_passphrase">
            ...
            />
            <ErrorFragment 
              errorMessage={errors?.ssh_passphrase} 
              fieldTouched={touchedFields.has('ssh_passphrase')}
            />
          </FormGroup>
        </React.Fragment>
      )}
      ...
      {typeValue === 'network' && (
        <React.Fragment>
          ...
          <FormGroup label="Become User" fieldId="become_user">
            ...
            />
            <ErrorFragment 
              errorMessage={errors?.become_user} 
              fieldTouched={touchedFields.has('become_user')}
            />
          </FormGroup>
          <FormGroup label="Become Password" fieldId="become_password">
            ...
            />
            <ErrorFragment 
              errorMessage={errors?.become_password} 
              fieldTouched={touchedFields.has('become_password')}
            />
          </FormGroup>
        </React.Fragment>
      )}
